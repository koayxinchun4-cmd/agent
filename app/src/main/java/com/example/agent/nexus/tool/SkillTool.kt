...
class SkillTool(
    ...
    override suspend fun execute(task: AgentTask): ToolResult {
        ...
        if (input.isEmpty()) return ToolResult.Failure("技能任務內容不能為空")

        return runCatching {
            when {
                input.contains("列表") || input.contains("list", ignoreCase = true) -> listSkills()
                input.contains("讀取") || input.contains("read", ignoreCase = true) -> readSkill(input)
                else -> listSkills()
            }
        }.getOrElse { ToolResult.Failure("技能工具執行失敗：${it.message ?: "未知錯誤"}", it) }
    }

    private fun listSkills(): ToolResult {
        val skills = registry.list()
        if (skills.isEmpty()) return ToolResult.Success("Nexus Skill Registry 目前沒有已安裝技能。")
    ...
    private fun readSkill(input: String): ToolResult {
        val id = input.substringAfter("讀取", "").trim()
            .ifEmpty { input.substringAfter("read", "").trim() }
        if (id.isEmpty()) return ToolResult.Failure("請提供要讀取的 Skill ID")
